...
static int j2k_write_siz(FILE *file, const j2k_basic_params *params)
{
    ...
    if (!j2k_write_marker(file, j2k_MARKER_SIZ)
        ...
        || !j2k_write_u16_be(file, 0u)
        || !j2k_write_u32_be(file, params->width)
        || !j2k_write_u32_be(file, params->height)
        || !j2k_write_u32_be(file, 0u)
        || !j2k_write_u32_be(file, 0u)
        || !j2k_write_u32_be(file, tile_width)
        || !j2k_write_u32_be(file, tile_height)
        || !j2k_write_u32_be(file, 0u)
        || !j2k_write_u32_be(file, 0u)
        || !j2k_write_u16_be(file, params->components))
    {
        return 0;
    }
    ...
    {
        if (!j2k_write_u8(file, 7u)
            || !j2k_write_u8(file, 1u)
            || !j2k_write_u8(file, 1u))
        {
            return 0;
        }
...
static int j2k_write_cod(FILE *file, const j2k_basic_params *params)
{
    ...
    if (!j2k_write_marker(file, j2k_MARKER_COD)
        || !j2k_write_u16_be(file, length)
        || !j2k_write_u8(file, scod)
        || !j2k_write_u8(file, 0u)
        || !j2k_write_u16_be(file, params->layers == 0u ? 1u : params->layers)
        || !j2k_write_u8(file, params->multiple_component_transform ? 1u : 0u)
        || !j2k_write_u8(file, params->decomposition_levels)
        || !j2k_write_u8(file, 4u)
        || !j2k_write_u8(file, 4u)
        || !j2k_write_u8(file, 0x04u)
        || !j2k_write_u8(file, params->reversible ? 1u : 0u))
    {
        return 0;
    }
...
static int j2k_write_rgn(FILE *file, const j2k_basic_params *params)
{
    ...
    for (component = 0u; component < params->components; ++component)
    {
        if (!j2k_write_marker(file, j2k_MARKER_RGN)
            || !j2k_write_u16_be(file, 5u)
            || !j2k_write_u8(file, component)
            || !j2k_write_u8(file, 0u)
            || !j2k_write_u8(file, params->roi_shift))
        {
            return 0;
        }
    }
...
)
{
    ...
    if (!j2k_write_marker(file, j2k_MARKER_QCC)
        || !j2k_write_u16_be(file, length)
        || !j2k_write_u8(file, (uint8_t)component)
        || !j2k_write_u8(file, 0x40u)
...
)
{
    ...
    return j2k_write_marker(file, j2k_MARKER_SOT)
        && j2k_write_u16_be(file, 10u)
        && j2k_write_u16_be(file, tile_index)
        && j2k_write_u32_be(file, tile_part_length)
        && j2k_write_u8(file, tile_part_index)
        && j2k_write_u8(file, tile_part_count);
}
