 ...
 *
 * IZ (Isolated Zero) dominates (~80%) — most coefficients are insignificant
...
#ifndef CODEC_SCAN_PROB_IZ
/** Probability weight for DIC_SCAN_TOKEN_IZ (Isolated Zero). */
#define CODEC_SCAN_PROB_IZ 0.80
#endif

#ifndef CODEC_SCAN_PROB_ZTR
/** Probability weight for DIC_SCAN_TOKEN_ZTR (Zerotree Root). */
#define CODEC_SCAN_PROB_ZTR 0.08
#endif

#ifndef CODEC_SCAN_PROB_POS
/** Probability weight for DIC_SCAN_TOKEN_POS (Positive Significant). */
#define CODEC_SCAN_PROB_POS 0.06
#endif

#ifndef CODEC_SCAN_PROB_NEG
/** Probability weight for DIC_SCAN_TOKEN_NEG (Negative Significant). */
#define CODEC_SCAN_PROB_NEG 0.06
#endif

static const double codec_scan_fixed_probs[DIC_SCAN_TOKEN_COUNT] = {
    CODEC_SCAN_PROB_IZ,  /* IZ  — Isolated Zero */
    CODEC_SCAN_PROB_ZTR, /* ZTR — Zerotree Root */
    CODEC_SCAN_PROB_POS, /* POS — Positive Significant */
    CODEC_SCAN_PROB_NEG  /* NEG — Negative Significant */
};
...
static dic_status codec_scan_encode_refinement_pass(
    const int32_t* plane, int bp, const codec_scan_sig_order* sig_order,
    size_t refinement_count, codec_scan_bit_writer* bit_writer) {
  size_t i;
 ...
 */
static dic_status codec_scan_resolution_rects(int width, int height, int levels,
                                              int resolution,
                                              dic_rect_i32 rects[3],
                                              int* count) {
  if (count == NULL || rects == NULL) return DIC_STATUS_INVALID_ARGUMENT;
 ...
 */
static int codec_scan_find_max_bitplane_in_rects(const int32_t* plane,
                                                 int width,
                                                 const dic_rect_i32* rects,
                                                 int rect_count) {
  uint32_t max_mag = 0u;
  int r;
  ...
  for (r = 0; r < rect_count; ++r) {
    ...
    for (y = 0; y < rect->height; ++y) {
      ...
      for (x = 0; x < rect->width; ++x) {
        size_t idx =
            ((size_t)(rect->y + y) * (size_t)width) + (size_t)(rect->x + x);
        uint32_t mag = plane[idx] < 0 ? (uint32_t)(-(plane[idx] + 1)) + 1u
                                      : (uint32_t)plane[idx];
        if (mag > max_mag) max_mag = mag;
      }
...
/**
 * @brief Encode significance pass for one bitplane over given subband
 * rectangles.
 *
...
static dic_status codec_scan_encode_significance_pass_nozt(
    int32_t* plane, int width, const dic_rect_i32* rects, int rect_count,
...
dic_status codec_scan_encode_subbands(const int32_t* plane, int width,
  ...
  *bitplane_count_out = 0;
  ...
  if (resolution < 0 || resolution > levels) return DIC_STATUS_INVALID_ARGUMENT;

  status = codec_scan_resolution_rects(width, height, levels, resolution, rects,
                                       &rect_count);
  if (status != DIC_STATUS_OK) return status;
  ...
  max_bp =
      codec_scan_find_max_bitplane_in_rects(plane, width, rects, rect_count);
  if (max_bp < 0) {
  ...
  for (bp = max_bp; bp >= 0; --bp) {
    ...
    status = codec_scan_encode_significance_pass_nozt(
        work_plane, width, rects, rect_count, threshold, significant,
        &token_buf, &sig_order);
    if (status != DIC_STATUS_OK) {
    ...
    if (status == DIC_STATUS_OK && sig_before > 0u) {
      ...
      status = codec_scan_encode_refinement_pass(work_plane, bp, &sig_order,
                                                 sig_before, &bit_writer);
      sig_order.count = saved;
    }
    if (status == DIC_STATUS_OK) codec_scan_bit_writer_flush(&bit_writer);
    ...
    /* Huffman-encode tokens with fixed table */
...
static dic_status codec_scan_decode_significance_pass_nozt(
    ...
    int out_width, const dic_rect_i32* rects, int rect_count, int32_t threshold,
    unsigned char* significant, codec_scan_sig_order* sig_order,
    int32_t* plane) {
  int r;
  dic_status status = DIC_STATUS_OK;
  ...
  for (r = 0; r < rect_count && status == DIC_STATUS_OK; ++r) {
    ...
    for (y = 0; y < rect->height && status == DIC_STATUS_OK; ++y) {
      ...
      for (x = 0; x < rect->width; ++x) {
        size_t idx =
            ((size_t)(rect->y + y) * (size_t)out_width) + (size_t)(rect->x + x);
...
dic_status codec_scan_decode_subbands(const codec_scan_bitplane* bitplanes,
  ...
  for (bp_idx = 0; bp_idx < decode_bitplanes; ++bp_idx) {
    ...
    status = codec_scan_decode_significance_pass_nozt(
        tokens, cur->dominant_token_count, &token_offset, out_width, rects,
        rect_count, threshold, significant, &sig_order, plane);
    free(tokens);
    ...
    if (cur->subordinate_bit_count > 0u) {
      ...
      codec_scan_bit_reader_init(&reader, cur->subordinate_bits,
                                 cur->subordinate_bit_count);
      status = codec_scan_decode_refinement_pass(&reader, bp, plane,
    ...
    if (last_decoded_bp > 0) {
      ...
      for (r = 0; r < rect_count; ++r) {
        ...
        for (y = 0; y < rect->height; ++y) {
          ...
          size_t idx;
          for (x = 0; x < rect->width; ++x) {
            idx = ((size_t)(rect->y + y) * (size_t)out_width) +
                  (size_t)(rect->x + x);
            if (plane[idx] > 0)
              plane[idx] += midpoint;
            else if (plane[idx] < 0)
