...
dic_status codec_subband_lowest_ll_rect(int width, int height, int levels,
                                        dic_rect_i32* rect) {
    dic_status status;

    if (rect == NULL) return DIC_STATUS_INVALID_ARGUMENT;
...
dic_status codec_subband_rect(int width, int height, int levels, int level,
    ...
    int low_width = dic_dwt53_low_size(current_width);
    int low_height = dic_dwt53_low_size(current_height);
    int high_width = dic_dwt53_high_size(current_width);
    int high_height = dic_dwt53_high_size(current_height);
