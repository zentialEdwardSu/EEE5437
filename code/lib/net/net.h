...
/**
 * Returns a human-readable peer address string ("IP:port"), or NULL when
 * the handle has no connected peer (e.g. still listening, or FILE transport).
 *
 * The returned pointer points to a static buffer.  Callers must copy the
 * string if they need it to survive past the next call.
 */
