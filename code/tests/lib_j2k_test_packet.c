...
int main(void)
{
    ...
    j2k_packet_header nonempty_payload;
    j2k_packet_codeblock codeblock;
    j2k_packet_codeblock prepared;
    j2k_packet_codeblock_payload contributions[2];
    j2k_codeblock_stream tagged_streams[3];
    j2k_codeblock_stream precinct_streams[4];
    j2k_packet_subband_payload tagged_subband;
    j2k_packet_subband_payload precinct_subband;
    j2k_packet_subband_layout parser_layout;
    j2k_packet_subband_layout precinct_parser_layout;
    j2k_packet_header_parser *parser = NULL;
    j2k_packet_parse_result parse_result;
    ...
    j2k_codeblock_stream eb_stream;
    j2k_packet_subband_payload eb_subband;
    ...
    j2k_packet_parse_result_init(&parse_result);
    j2k_codeblock_stream_init(&eb_stream);
    ...
    for (i = 0u; i < 3u; ++i)
        j2k_codeblock_stream_init(tagged_streams + i);
    for (i = 0u; i < 4u; ++i)
        j2k_codeblock_stream_init(precinct_streams + i);

    DIC_EXPECT(j2k_packet_build_empty_header(&header) == DIC_STATUS_OK);
    ...
    j2k_packet_header_init(&header);
    for (i = 0u; i < 9u; ++i)
        DIC_EXPECT(j2k_packet_header_append_bit(&header, 1u) == DIC_STATUS_OK);
    ...
    j2k_packet_header_init(&header);
    DIC_EXPECT(j2k_packet_calculate_lblock_increment(3u, 1u, 7u, &increment) == DIC_STATUS_OK);
    DIC_EXPECT(increment == 0u);
    DIC_EXPECT(j2k_packet_calculate_lblock_increment(3u, 1u, 8u, &increment) == DIC_STATUS_OK);
    DIC_EXPECT(increment == 1u);
    DIC_EXPECT(j2k_packet_calculate_lblock_increment(3u, 9u, 31u, &increment) == DIC_STATUS_OK);
    DIC_EXPECT(increment == 0u);
    ...
    DIC_EXPECT(j2k_packet_header_append_bit(&header, 1u) == DIC_STATUS_OK);
    DIC_EXPECT(j2k_packet_header_append_codeblock(&header, &codeblock) == DIC_STATUS_OK);
    DIC_EXPECT(j2k_packet_header_finish(&header) == DIC_STATUS_OK);
    DIC_EXPECT(header.size > 0u);
    j2k_packet_header_free(&header);

    j2k_packet_header_init(&nonempty_payload);
    DIC_EXPECT(j2k_packet_build_single_codeblock_payload(
        &codeblock,
        codeword,
        sizeof(codeword),
        &nonempty_payload
    ...
    j2k_packet_header_free(&nonempty_payload);

    DIC_EXPECT(j2k_packet_prepare_codeblock(&prepared, 0u, 9u, sizeof(codeword)) == DIC_STATUS_OK);
    ...
    j2k_packet_header_init(&header);
    DIC_EXPECT(j2k_packet_header_append_codeblock(&header, &prepared) == DIC_STATUS_OK);
    DIC_EXPECT(j2k_packet_header_finish(&header) == DIC_STATUS_OK);
    ...
    DIC_EXPECT(j2k_packet_prepare_codeblock(&prepared, 1u, 2u, sizeof(second_codeword)) == DIC_STATUS_OK);
    ...
    j2k_packet_header_init(&nonempty_payload);
    DIC_EXPECT(j2k_packet_build_codeblock_payload(contributions, 2u, &nonempty_payload) == DIC_STATUS_OK);
    ...
    DIC_EXPECT(j2k_packet_build_empty_lrcp_payload(3u, 5u, 1u, &payload) == DIC_STATUS_OK);
    ...
    j2k_packet_header_init(&payload);
    DIC_EXPECT(j2k_packet_build_tagged_ebcot_payload(&tagged_subband, 1u, &payload) == DIC_STATUS_OK);
    ...
    DIC_EXPECT(j2k_packet_header_parser_create(&precinct_parser_layout, 1u, 0, &parser) == DIC_STATUS_OK);
    j2k_packet_header_init(&payload);
    DIC_EXPECT(j2k_packet_build_tagged_ebcot_layer_payload_with_header_size(
        &precinct_subband,
        1u,
        0u,
        1u,
    ...
    DIC_EXPECT(j2k_packet_header_parser_parse(parser, payload.data, payload.size, 0u, &parse_result) == DIC_STATUS_OK);
    ...
    j2k_packet_header_free(&payload);
    j2k_packet_header_parser_destroy(parser);
    parser = NULL;
    ...
    j2k_packet_header_init(&payload);
    DIC_EXPECT(j2k_packet_build_ebcot_payload(&eb_stream, 1u, &payload) == DIC_STATUS_OK);
    ...
    DIC_EXPECT(j2k_packet_header_parser_create(&parser_layout, 1u, 1, &parser) == DIC_STATUS_OK);
    ...
    DIC_EXPECT(j2k_packet_header_parser_parse(parser, payload.data, payload.size, 1u, &parse_result) == DIC_STATUS_OK);
    DIC_EXPECT(parse_result.is_empty == 0);
    DIC_EXPECT(parse_result.packet_header_size == layer_header_size);
    ...
    j2k_packet_header_free(&payload);
    j2k_packet_parse_result_free(&parse_result);
    j2k_packet_header_parser_destroy(parser);
    j2k_codeblock_stream_free(&eb_stream);
