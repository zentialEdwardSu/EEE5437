...
int main(void)
{
    ...
    j2k_mq_stream biased_stream;
    j2k_mq_stream stress_stream;
    j2k_mq_stream custom_stream;
    size_t i;
    j2k_mq_context_state state;
    j2k_mq_context_state initial_states[19];
    j2k_mq_registers registers;
    const j2k_mq_table_entry *table = j2k_mq_table();

    j2k_mq_stream_init(&stream);
    j2k_mq_stream_init(&biased_stream);
    j2k_mq_stream_init(&stress_stream);
    j2k_mq_stream_init(&custom_stream);
    DIC_EXPECT(j2k_mq_table_size() == 47u);
    ...
    DIC_EXPECT(j2k_mq_initenc_registers(&registers, 0u) == DIC_STATUS_OK);
    DIC_EXPECT(registers.a == 0x8000u);
    DIC_EXPECT(registers.c == 0u);
    DIC_EXPECT(registers.ct == 12u);
    DIC_EXPECT(j2k_mq_initenc_registers(&registers, 0xffu) == DIC_STATUS_OK);
    DIC_EXPECT(registers.a == 0x8000u);
    DIC_EXPECT(registers.c == 0u);
    DIC_EXPECT(registers.ct == 13u);

    DIC_EXPECT(j2k_mq_contexts_init(&state, 1u) == DIC_STATUS_OK);
    DIC_EXPECT(state.index == 0u);
    DIC_EXPECT(state.mps == 0u);
    DIC_EXPECT(j2k_mq_context_update(&state, 0u) == DIC_STATUS_OK);
    DIC_EXPECT(state.index == 1u);
    DIC_EXPECT(state.mps == 0u);
    DIC_EXPECT(j2k_mq_context_update(&state, 1u) == DIC_STATUS_OK);
    DIC_EXPECT(state.index == 6u);

    DIC_EXPECT(j2k_mq_encode_decisions(
        contexts,
        decisions,
    ...
    DIC_EXPECT(j2k_mq_decode_decisions(
        &stream,
        contexts,
    ...
    j2k_mq_stream_free(&biased_stream);
    ...
    j2k_mq_stream_free(&stress_stream);

    DIC_EXPECT(j2k_mq_contexts_init(initial_states, sizeof(initial_states) / sizeof(initial_states[0])) == DIC_STATUS_OK);
    ...
    DIC_EXPECT(j2k_mq_encode_decisions_with_states(
        initial_states,
    ...
    DIC_EXPECT(j2k_mq_decode_decisions_with_states(
        &custom_stream,
        initial_states,
        sizeof(initial_states) / sizeof(initial_states[0]),
        custom_contexts,
    ...
    j2k_mq_stream_free(&custom_stream);
