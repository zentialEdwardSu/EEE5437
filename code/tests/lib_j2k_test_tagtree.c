...
int main(void)
{
    ...
    DIC_EXPECT(dic_j2k_tagtree_alloc(&tree, 2, 2) == DIC_STATUS_OK);
    DIC_EXPECT(dic_j2k_tagtree_set(&tree, 1, 0, 2u) == DIC_STATUS_OK);
    DIC_EXPECT(dic_j2k_tagtree_encode_leaf(&tree, 1, 0, 1u, &header) == DIC_STATUS_OK);
    ...
    dic_j2k_packet_header_init(&header);
    DIC_EXPECT(dic_j2k_tagtree_encode_leaf(&tree, 1, 0, 2u, &header) == DIC_STATUS_OK);
    DIC_EXPECT(dic_j2k_packet_header_finish(&header) == DIC_STATUS_OK);
    DIC_EXPECT(header.size == 1u);
