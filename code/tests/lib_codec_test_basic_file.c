...
int main(void) {
    ...
    /* Encode and write */
    {
        ...
        DIC_EXPECT(codec_basic_encode_image(source, 32, 32, 1, 3, 4, 0,
                                            &encoded) == DIC_STATUS_OK);
        DIC_EXPECT(codec_basic_write_file(TEST_FILE, &encoded) ==
                   DIC_STATUS_OK);
    ...
    /* Partial read — resolution 1 only (LL + 1 high-pass level => 8x8) */
    {
        ...
        DIC_EXPECT(codec_basic_read_file_resolution(TEST_FILE, 1, &encoded) ==
                   DIC_STATUS_OK);
        ...
        DIC_EXPECT(encoded.channel_streams[0].resolutions[2].num_bitplanes ==
                   0);
        DIC_EXPECT(encoded.channel_streams[0].resolutions[3].num_bitplanes ==
                   0);

        DIC_EXPECT(codec_basic_decode_image(&encoded, 1, 0, &decoded) ==
                   DIC_STATUS_OK);
    ...
    /* Partial read — resolution 0 (LL only => 4x4) */
    {
        ...
        DIC_EXPECT(codec_basic_read_file_resolution(TEST_FILE, 0, &encoded) ==
                   DIC_STATUS_OK);
        DIC_EXPECT(codec_basic_decode_image(&encoded, 0, 0, &decoded) ==
                   DIC_STATUS_OK);
