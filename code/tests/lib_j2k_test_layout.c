...
int main(void)
{
    ...
    DIC_EXPECT(j2k_resolution_subband_count(5, 0, &subbands) == DIC_STATUS_OK);
    DIC_EXPECT(subbands == 1);
    DIC_EXPECT(j2k_resolution_subband_count(5, 3, &subbands) == DIC_STATUS_OK);
    DIC_EXPECT(subbands == 3);

    DIC_EXPECT(j2k_codeblock_grid_for_subband(
        130,
        97,
        3,
        3,
    ...
    DIC_EXPECT(j2k_codeblock_rect(&grid, grid.blocks_x - 1, grid.blocks_y - 1, &rect) == DIC_STATUS_OK);
    ...
    grid.blocks_y = 1;
    grid.block_count = 4u;
    DIC_EXPECT(j2k_precinct_grid_for_subband(&grid, 1, 6u, 6u, &precinct_grid) == DIC_STATUS_OK);
    ...
    DIC_EXPECT(j2k_precinct_codeblock_window(
        &precinct_grid,
        2,
        0,
        &first_block_x,
    ...
    DIC_EXPECT(j2k_packet_count_lrcp(3, 5, 2, &packets) == DIC_STATUS_OK);
